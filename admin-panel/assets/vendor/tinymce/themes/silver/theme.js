...
(function () {
  ...
  const isBottomToTopDir = (el) =>
    closest$2(
      ...
      (current) =>
        isElement$1(current) &&
        get$f(current, "data-alloy-vertical-dir") ===
  ...
  const virtualReplaceChildren = (component, newSpecs, buildNewChildren) => {
    const subs = component.components();
    const existingComps = bind$3(newSpecs, (spec) =>
      getPremade(spec).toArray(),
    );
  ...
  const execute = (component, simulatedEvent, menuConfig) =>
    menuConfig.focusManager
      .get(component)
      .bind((focused) =>
        menuConfig.execute(component, simulatedEvent, focused),
      );
  ...
  const makeMenu = (
    ...
    const layouts =
      menuSpec.type === "horizontal"
        ...
        : {};
    const isFirstTierSubmenu = (triggeringPaths) =>
      triggeringPaths.length === 2;
  ...
  const register$e = (editor) => {
    ...
    const stringOrFalseProcessor = (value) =>
      isString(value) || value === false;
  ...
  const toggleMenuItemSchema = objOf(
    [type, optionalIcon, active, onSetup, onAction].concat(
      commonMenuItemFields,
    ),
  );
  ...
  const register$b = (editor, sharedBackstage) => {
    ...
    const getCombinedItems = (matches) => {
      const columns = findMap(matches, (m) => Optional.from(m.columns)).getOr(
        1,
      );
  ...
  const getBounds = (component) =>
    component.element.dom.getBoundingClientRect();
  ...
  const sketch$2 = (detail, components, _spec, _externals) => {
    ...
    const getRightEdge = (component) =>
      getPart(component, detail, "right-edge");
  ...
  const init$6 = (spec) => {
    ...
    const readState = () =>
      `docked:  ${docked.get()}, visible: ${visible.get()}, modes: ${modes.get().join(",")}`;
    return nu$8({
      isDocked: docked.get,
  ...
  const createMenuButton = (spec) =>
    asRaw("menubutton", MenuButtonSchema, spec);
  ...
  const matchTargetWith = (elem, candidates) => {
    ...
    const { pass, fail } = partition$3(
      ctxs,
      (t) => t.type === "contexttoolbar",
    );
    return {
